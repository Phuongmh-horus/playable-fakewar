...
- For per-unit VAT death visuals, explicitly verify the batching path at runtime: a shared material or batch-wide shader property must never carry state that differs between living and dead units. Prefer an instance-safe render-data path; otherwise remove the visual rather than corrupting all units.
- When a render-state change leaks across batched VAT units, prove the isolation on simultaneous living and dead units before retaining the effect; preserve batching by putting the state in per-instance vertex/render data, or remove that effect.
- For a tutorial swipe that must transition into live steering, trigger gameplay as soon as the valid directional threshold is crossed while the pointer is held; deciding only on release turns every swipe into a delayed tap.
