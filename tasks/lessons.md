...
- Do not claim a performance improvement from hiding a Profiler allocation alone. Verify that the hot path performs less work and that the before/after profiler call tree reflects the intended structural change.
- Before replacing a gameplay `Spawn` call with strict `TrySpawn` for Web/Luna, prove that the same prefab pool is populated before the first use. If startup prewarm can be skipped or fail, retain a bounded lazy-instantiation fallback; also verify that any direct-damage fallback has its required target cache populated.
- In this project's Web/Luna gameplay paths, character and projectile creation must preserve `Spawn` fallback semantics; never replace them with pool-only `TrySpawn`, because a temporary pool miss silently removes characters or shots.
