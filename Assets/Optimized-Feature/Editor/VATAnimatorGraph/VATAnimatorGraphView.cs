...
namespace OptimizedFeature.Editor.VATAnimator
{
    ...
    internal sealed class VATAnimatorGraphView : GraphView
    {
        ...
        {
            // Node data is expressed in contentViewContainer coordinates. The
            // local pointer position ignores the current pan/zoom transform and
            // appeared offset after the Blackboard changed the workspace layout.
            Vector2 menuPosition = contentViewContainer.WorldToLocal(evt.mousePosition);
            ...
            evt.menu.AppendAction("Add Transition", action => owner.AddTransitionNode(menuPosition));
            evt.menu.AppendAction("Add Blend Tree", action => owner.AddBlendTreeNode(menuPosition));
