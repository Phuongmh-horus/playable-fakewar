...
namespace OptimizedFeature.Editor.VATAnimator
{
    ...
    /// remains VATAssetDataSO.Clips while animator data is resolved through its linked
    /// VATAssetAnimatorSO.
    /// </summary>
    ...
    /// <summary>
    /// Editor adapter over VATAssetDataSO and its linked VATAssetAnimatorSO.
    ///
    /// This class is intentionally not a persisted graph asset. It exists only to keep the
    /// GraphView code readable while clip data remains on the selected VATAssetDataSO and
    /// animator data is read from or written to its linked VATAssetAnimatorSO.
    /// </summary>
    internal sealed class VATAnimatorGraphAsset : ScriptableObject
    {
        ...
        public List<string> GetMissingClipKeys()
        {
            List<string> missing = new List<string>();
            if (_sourceVATAsset == null) return missing;

            HashSet<string> validKeys = new HashSet<string>(StringComparer.Ordinal);
            if (_sourceVATAsset.Clips != null)
            {
                for (int i = 0; i < _sourceVATAsset.Clips.Count; i++)
                {
                    VATClipInfo clip = _sourceVATAsset.Clips[i];
                    if (clip != null && !string.IsNullOrEmpty(clip.ClipName))
                    {
                        validKeys.Add(MakeClipKey(clip));
                    }
                }
            }

            Action<string> addMissing = value =>
            {
                if (!string.IsNullOrEmpty(value) && !missing.Contains(value)) missing.Add(value);
            };

            if (!string.IsNullOrEmpty(defaultClipKey) && !validKeys.Contains(defaultClipKey))
            {
                addMissing("Default: " + defaultClipKey);
            }
            ...
            if (nodes != null)
            {
                for (int i = 0; i < nodes.Count; i++)
                {
                    VATAnimatorNodeData node = nodes[i];
                    if (node != null && node.nodeType == VATAnimatorNodeType.Clip &&
                        !validKeys.Contains(node.clipKey))
                    {
                        addMissing("Clip Node: " + node.clipKey);
                    }
                }
            }
            ...
            if (blendTrees != null)
            {
                for (int i = 0; i < blendTrees.Count; i++)
                {
                    ...
                    {
                        VATAnimatorBlendChildData child = tree.children[c];
                        if (child != null && !validKeys.Contains(child.clipKey))
                        {
                            addMissing("Blend Tree '" + tree.title + "': " + child.clipKey);
                        }
                    }
                }
            }
            ...
            if (transitions != null)
            {
                for (int i = 0; i < transitions.Count; i++)
                {
                    ...
                    if (transition == null) continue;
                    if (transition.fromStateHash != 0 && FindClipByHash(transition.fromStateHash) == null)
                    {
                        addMissing("Transition '" + transition.title + "' source hash: " + transition.fromStateHash);
                    }
                    if (transition.toStateHash != 0 && FindClipByHash(transition.toStateHash) == null)
                    {
                        addMissing("Transition '" + transition.title + "' target hash: " + transition.toStateHash);
                    }
                }
            }
        ...
        private void EnsureDefaultEdge(HashSet<string> validKeys)
        {
            ...
            if (existingEdge != null)
            {
                ...
                else
                {
                    ...
                    defaultClipKey = string.Empty;
                    _sourceVATAsset.SetAnimatorDefaultStateName(0);
                }
                return;
            }
        ...
        private void SyncRuntimeDefaultState()
        {
            if (_sourceVATAsset == null) return;
            VATAnimatorClipData clip = FindClipByKey(defaultClipKey);
            _sourceVATAsset.SetAnimatorDefaultStateName(clip == null ? 0 : clip.stateHash);
        }
        ...
        public bool ClearRuntimeEdgeMapping(VATAnimatorEdgeData edge)
        {
            ...
            {
                defaultClipKey = string.Empty;
                if (_sourceVATAsset != null) _sourceVATAsset.SetAnimatorDefaultStateName(0);
                return true;
            }
        ...
        public VATAnimatorNodeData AddTransitionNode(Vector2? position = null)
        {
            EnsureLists();
            ...
            VATAnimatorNodeData node = new VATAnimatorNodeData
            {
                ...
                position = position ?? new Vector2(580f + (transitions.Count - 1) * 30f, 400f)
            };
            nodes.Add(node);
            return node;
        }

        public VATAnimatorNodeData AddBlendTreeNode(Vector2? position = null)
        {
            EnsureLists();
            ...
            VATAnimatorNodeData node = new VATAnimatorNodeData
            {
                ...
                position = position ?? new Vector2(820f + (blendTrees.Count - 1) * 30f, 400f)
            };
            nodes.Add(node);
            return node;
        }
