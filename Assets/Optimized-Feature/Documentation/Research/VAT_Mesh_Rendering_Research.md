...
> **Lưu ý khi đối chiếu với source:** tài liệu này mô tả định hướng nghiên cứu và có một số tên component/khẳng định hiệu năng từ giai đoạn trước. Implementation hiện tại dùng `VAT_RenderComponent`, `VATWeaponRenderComponent`, `VATSystem` và runtime mesh batching; ranh giới detachable được mô tả trong [README](../../README.md).
...
### A. `VAT_RenderComponent` ([VAT_RenderComponent.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Runtime/Rendering/VAT_RenderComponent.cs))
- **Đại diện thay thế cho**: Components `Animator` và `SkinnedMeshRenderer` mặc định của Unity.
...
- **Chức năng**: Tính toán frame index theo thời gian thực và điều phối giá trị `_FrameIndexLower`, `_FrameIndexUpper`, `_BlendWeight` cho shader VAT.

### B. `VATWeaponRenderComponent` ([VATWeaponRenderComponent.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Runtime/Rendering/VATWeaponRenderComponent.cs))
...
### C. `VATSystem` ([VATSystem.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Runtime/Systems/VATSystem.cs))
...
Tool `VATBakeToolWindow` ([VATBakeToolWindow.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Editor/Baking/VATBakeToolWindow.cs)) tự động quét các Material của nhân vật và chèn file `VAT_Core.cginc` vào ShaderX mà không cần thao tác tay thủ công.
...
1. Unified Animator/Renderer Bridge: [VAT_RenderComponent.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Runtime/Rendering/VAT_RenderComponent.cs)
2. Weapon VAT Renderer: [VATWeaponRenderComponent.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Runtime/Rendering/VATWeaponRenderComponent.cs)
3. Core Execution System: [VATSystem.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Runtime/Systems/VATSystem.cs)
4. Structure Data Model: [VATAnimStateData.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Runtime/Animation/VATAnimStateData.cs) & [VATAssetDataSO.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Runtime/Assets/VATAssetDataSO.cs)
5. HLSL Core Include: [VAT_Core.cginc](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Shaders/Includes/VAT_Core.cginc)
6. Bake Tool Simulation: [VATBakeToolWindow.cs](file:///c:/PlayableProject/Playable_RPG_Game_Checker/Assets/Optimized-Feature/Editor/Baking/VATBakeToolWindow.cs)
...
* **Tính toán World Bounds chính xác**: `VAT_RenderComponent` tự tính toán World Space Bounding Box dựa trên thông số `BoundingMin` và `BoundingMax` nướng sẵn từ ScriptableObject, kết hợp với Scale/Position thực tế của nhân vật và cộng thêm $15\%$ padding lề để tránh hiện tượng mất hình đột ngột (popping) ở mép màn hình.
