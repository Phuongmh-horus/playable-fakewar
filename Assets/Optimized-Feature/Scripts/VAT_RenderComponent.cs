...
namespace OptimizedFeature.Scripts
{
    ...
    public class VAT_RenderComponent : MonoBehaviour
    {
        ...
        public int DefaultStateHash => _vatAssetData == null ? 0 : _vatAssetData.DefaultStateName;
        ...
        {
            if (_vatAssetData == null || _vatAssetData.DefaultStateName == 0) return;
            Play(_vatAssetData.DefaultStateName);
        }
        ...
        public void RefreshWeaponSubRenders()
        {
        ...
        public bool SetWeaponAsset(VATWeaponAssetSO weaponAsset)
        {
            if (weaponAsset != null && !IsWeaponFrameManifestCompatible(weaponAsset))
            {
                ...
                Debug.LogError(
                    $"[VAT_RenderComponent] Weapon VAT '{weaponAsset.name}' has " +
                    $"{weaponAsset.TotalFrames} frames but Body VAT '{bodyName}' has " +
                    $"{bodyFrames}, or its clip ranges differ. Weapon switching was rejected because the frame manifests differ.",
                    gameObject);
                return false;
            }
            ...
            _weaponRenderComponents[0].SetFrameSource(this);
            _weaponRenderComponents[0].SetWeaponAsset(weaponAsset);
            return true;
        }
