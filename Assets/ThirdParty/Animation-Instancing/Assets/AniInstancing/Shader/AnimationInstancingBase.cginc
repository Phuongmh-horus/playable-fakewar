...
half4x4 loadMatFromTexture(uint frameIndex, uint boneIndex)
{
	...
	uv.x = _boneTextureBlockWidth * ((int)frameIndex - (int)blockCount * uv.y);

	int matCount = (int)((uint)_boneTextureBlockWidth / 4u);
...
half4 skinning(inout appdata_full v)
{
	...
	if (preAniFrame >= 0.0f && progress < 1.0f)
	{
...
half4 skinningPosition(appdata_full v)
{
	fixed4 w = v.color;
	half4 bone = half4(v.texcoord2.x, v.texcoord2.y, v.texcoord2.z, v.texcoord2.w);
#if (SHADER_TARGET < 30 || SHADER_API_GLES)
	...
	if (preAniFrame >= 0.0f && progress < 1.0f)
	{
		...
		localToWorldMatrixPreAni += loadMatFromTexture(preAniFrame, bone.z) * max(0, w.z);
		localToWorldMatrixPreAni += loadMatFromTexture(preAniFrame, bone.w) * max(0, w.w);
		localPos = lerp(mul(v.vertex, localToWorldMatrixPreAni), localPos, progress);
	}
	return localPos;
}
...
half4 skinningShadow(inout appdata_full v)
{
	...
	if (preAniFrame > 0.0f && progress < 1.0f)
	{
		...
		localPos = lerp(localPos, mul(v.vertex, localToWorldMatrixPreAni), 1.0f - progress);
	}
