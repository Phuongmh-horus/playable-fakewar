...
namespace AnimationInstancing
{
    public class AnimationManager : Singleton<AnimationManager>
    {
        ...
        public InstanceAnimationInfo FindAnimationInfo(GameObject prefab, AnimationInstancing instance)
        {
            ...
            if (instance != null && instance.animationData != null)
            {
                using (BinaryReader reader = new BinaryReader(new MemoryStream(instance.animationData.bytes)))
                {
        ...
        private InstanceAnimationInfo CreateAnimationInfoFromFile(GameObject prefab)
        {
            ...
            path = GetAnimationTextureFilePath(prefab.name);
            ...
            while (!w.isDone) { }
            if (!string.IsNullOrEmpty(w.error) || w.bytes == null || w.bytes.Length == 0)
            {
                Debug.LogError("Can't load Android animation data for '" + prefab.name + "': " + w.error);
                return null;
            }
//             BinaryReader reader = new BinaryReader(new MemoryStream(w.bytes));
...
#if UNITY_EDITOR || UNITY_STANDALONE_WIN
        private string GetAnimationTextureFilePath(string prefabName)
        {
            string fileName = prefabName + ".bytes";
            string defaultPath = Path.Combine(Application.dataPath, "AnimationTexture", fileName);
