...
namespace AnimationInstancing
{
    public class AnimationGenerator : EditorWindow
    {
        ...
        private Dictionary<string, int> bakeOrders = new Dictionary<string, int>();
        ...
        private void OnGUI()
        {
            ...
            if (generatedPrefab)
            {
                ...
                string[] clipNames = generateAnims.Keys
                    .OrderBy(clipName => clipName)
                    .ToArray();
                int totalFrames = 0;
                ...
                EditorGUILayout.LabelField("Bake Order (smaller value bakes first)");
                ...
                foreach (var clipName in clipNames)
                {
                    ...
                    GUILayout.BeginHorizontal();
                    {
                        ...
                        bakeOrders[clipName] = Mathf.Max(0, EditorGUILayout.IntField(bakeOrders[clipName], GUILayout.Width(50)));
        ...
        void BakeWithAnimator()
        {
            if (generatedPrefab != null)
            {
                ...
                generateInfo.Sort((left, right) =>
                {
                    int orderCompare = bakeOrders[left.info.animationName].CompareTo(bakeOrders[right.info.animationName]);
                    return orderCompare != 0
                        ? orderCompare
                        : string.CompareOrdinal(left.info.animationName, right.info.animationName);
                });
                int animationIndex = 0;
                for (int i = 0; i < generateInfo.Count; ++i)
                {
                    generateInfo[i].info.animationIndex = animationIndex;
                    animationIndex += generateInfo[i].info.totalFrame;
                }
        ...
        private List<AnimationClip> GetClips(bool bakeFromAnimator)
        {
            ...
            foreach (AnimationClip clip in clips)
            {
                if (generateAnims.ContainsKey(clip.name) == false)
                {
                    ...
                    bakeOrders.Add(clip.name, bakeOrders.Count);
                }
            }
            ...
            for (int i = 0; i < distinctClips.Count; i++)
            {
                if (generateAnims.ContainsKey(distinctClips[i].name) == false)
                {
                    ...
                    bakeOrders.Add(distinctClips[i].name, bakeOrders.Count);
                }
            }
