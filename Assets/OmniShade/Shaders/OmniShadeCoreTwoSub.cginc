...
half4 _MatCapMaskTex_ST;
half _MatCapMaskMode;
half4 _MatCapColor;
...
float4 _OmniShadePlayerInteractor;
float4 _OmniShadePlayerInteractorParams;
...
float3 PlantSway(appdata_full v) {
	...
	#endif
	#if PLAYER_BEND
		if (_OmniShadePlayerInteractor.w > 0.5 && _OmniShadePlayerInteractorParams.x > 0.0001 && _OmniShadePlayerInteractorParams.y > 0.0001) {
			float3 worldPos = TransformObjectToWorld(pos).xyz;
			float2 deltaXZ = worldPos.xz - _OmniShadePlayerInteractor.xz;
			float radius = _OmniShadePlayerInteractorParams.x;
			float strength = _OmniShadePlayerInteractorParams.y;
			float distanceSq = dot(deltaXZ, deltaXZ);
			if (radius > 0.0001 && distanceSq < radius * radius) {
				float distance = sqrt(max(distanceSq, 0.000001));
				float2 bendDirection = deltaXZ / distance;
				float falloff = 1 - saturate(distance / radius);
				falloff *= falloff;
				#if _PLANTTYPE_LEAF
					half bendMask = 1;
				#elif _PLANTTYPE_PLANT
					half bendMask = saturate(max(0, pos.y - _PlantBaseHeight) * _PlayerBendHeightScale);
				#elif _PLANTTYPE_VERTEX_COLOR_ALPHA
					half bendMask = v.color.a;
				#else
					half bendMask = 1;
				#endif
				float2 bendOffsetXZ = bendDirection * (falloff * strength * bendMask);
				worldPos.xz += bendOffsetXZ;
				worldPos.y -= length(bendOffsetXZ) * 0.35;
				pos = mul(GetWorldToObjectMatrix(), float4(worldPos, 1)).xyz;
			}
		}
	#endif
	return pos;
...
half4 frag (v2f i) : COLOR {
	...
	// BASE
		...
		half matCapMainAlpha = saturate(col_base.a);
		...
		half matCapMainAlpha = saturate(colX.a * blend.x + colZ.a * blend.z + colYbottom.a * (blend.y + blend.w));
		...
		#endif
		half rimMaskTex = tex2D(_RimMaskTex, TRANSFORM_TEX(uv, _RimMaskTex)).r;
		half rimMask = _RimMaskMode == 0 ? rimMaskTex : matCapMainAlpha;
		rim *= rimMask;
	...
	// LIGHTING
		...
		#endif
		half matCapMaskTex = tex2D(_MatCapMaskTex, TRANSFORM_TEX(uv, _MatCapMaskTex)).r;
		half matCapMask = _MatCapMaskMode == 0 ? matCapMaskTex : matCapMainAlpha;
		col = lerp(col.rgb, BlendMatCap(col.rgb, col_matcap), matCapMask);
	#endif
	#if SPECULAR
