//------------------------------------
//             OmniShade
//     Natural Burn Dissolve
//     (Fix: i.uv & Natural Edge)
//------------------------------------

Shader "OmniShade/Standard_Natural_Dissolve" {
	Properties {
        ...
        // ========================================================
        // [MODIFIED] NATURAL DISSOLVE (KIỂU TỰ NHIÊN / ĂN MÒN)
        // ========================================================
        [Header(Natural Dissolve)]
        [Toggle(DISSOLVE_ENABLED)] _DissolveEnabled ("Enable Dissolve", Float) = 0
        _DissolveTex ("Dissolve Noise (Bắt buộc)", 2D) = "white" {}
        _DissolveAmount ("Dissolve Amount", Range(0, 1)) = 0

        // Viền cháy/tối màu thay vì phát sáng
        _DissolveBurnSize ("Burn Edge Size", Range(0, 0.5)) = 0.1
        _DissolveBurnColor ("Burn Color (Dark)", Color) = (0.1, 0.1, 0.1, 1) // Màu xám đen mặc định

        _DissolveTiling ("Noise Tiling (Tăng lên để hết sọc)", Float) = 10
        // ========================================================
    ...
    Subshader {
        ...
        Pass {
            ...
            #pragma shader_feature DISSOLVE_ENABLED
            ...
            // [MỚI] HÀM XỬ LÝ DISSOLVE - KIỂU BURN (TỰ NHIÊN)
            fixed4 frag_dissolve(v2f i) : SV_Target {
                fixed4 finalColor = frag(i);

                #if defined(DISSOLVE_ENABLED)
                    // 1. Lấy Noise
                    float noise = tex2D(_DissolveTex, i.uv.xy * _DissolveTiling).r;

                    // 2. Cắt bỏ phần đã tan biến
                    clip(noise - _DissolveAmount);

                    // 3. Tính toán viền cháy (Shadow / Burn Edge)
                    // Tạo một vùng đệm (Edge) giữa phần bị cắt và phần còn lại
                    // smoothstep giúp làm mềm mép, trông tự nhiên hơn là step cứng
                    float edge = smoothstep(_DissolveAmount, _DissolveAmount + _DissolveBurnSize, noise);

                    // 4. Pha trộn màu:
                    // Ở mép vết cắt (edge = 0), màu sẽ là BurnColor (đen/xám)
                    // Ở xa vết cắt (edge = 1), màu sẽ là màu gốc (finalColor)
                    if (_DissolveAmount > 0)
                    {
                        finalColor.rgb = lerp(_DissolveBurnColor.rgb, finalColor.rgb, edge);
                    }
                #endif
